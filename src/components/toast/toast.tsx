...
const CLR={success:'#17AB78',danger:'#FF0000',warning:'#E3A92D',info:'#005BA6'};
...
export const Toast:React.FC<ToastProps>=({variant='info',message,description,duration=4000,onDismiss,visible=true})=>{
  ...
  const bg=BG[variant],clr=CLR[variant];
  ...
  return(<div role="alert" aria-live="polite" style={{position:'fixed',bottom:24,right:24,zIndex:9999,maxWidth:420,display:'flex',alignItems:'flex-start',gap:12,padding:'14px 16px',backgroundColor:bg,borderRadius:8,boxShadow:'0 4px 12px rgba(0,47,72,0.15)',fontFamily:FONT,color:clr,opacity:isExiting?0:1,transition:'opacity 200ms ease'}}>
    <div style={{flex:1,minWidth:0}}>
      <div style={{fontSize:14,fontWeight:600,color:clr}}>{message}</div>
      {description&&<div style={{fontSize:13,color:clr,opacity:0.85,marginTop:2}}>{description}</div>}
    </div>
    <button onClick={handleDismiss} aria-label="Dismiss" style={{background:'none',border:'none',cursor:'pointer',color:clr,opacity:0.7,padding:2,fontSize:14}}>x</button>
  </div>);
};
