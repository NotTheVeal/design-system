...
// SVG icons — no unicode, no emoji
const SuccessIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
    <circle cx="9" cy="9" r="9" fill="#0E7C55"/>
...
const WarningIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
    <path d="M9 1L1 16h16L9 1z" fill="#E3A92D"/>
    <path d="M9 7v4M9 13h.01" stroke="#fff" strokeWidth="1.5" strokeLinecap="round"/>
  </svg>
);
const InfoIcon = () => (
  <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
    <circle cx="9" cy="9" r="9" fill="#005BA6"/>
    <path d="M9 8v5M9 6h.01" stroke="#fff" strokeWidth="1.5" strokeLinecap="round"/>
  </svg>
);
const CloseIcon = () => (
  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
    <path d="M1 1l12 12M13 1L1 13" stroke="#949494" strokeWidth="1.5" strokeLinecap="round"/>
  </svg>
);
...
const ToastItem: React.FC<{ toast: ToastMessage; onDismiss: (id: string) => void }> = ({ toast, onDismiss }) => {
  ...
  return (
    <div style={{
      ...
      borderRadius: 4,
      boxShadow: '0 4px 16px rgba(0,47,72,.12)',
      minWidth: 300,
      maxWidth: 400,
      overflow: 'hidden',
      fontFamily: font,
      ...
      <div style={{ padding: '12px 14px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <button
          onClick={dismiss}
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', flexShrink: 0, marginTop: 2 }}
          aria-label="Dismiss"
        >
          <CloseIcon />
        </button>
      ...
      {!!dur && (
        <div style={{ height: 3, background: '#F1F1F1' }}>
          <div style={{ height: '100%', background: c.bar, width: progress + '%', transition: 'width 50ms linear' }} />
        </div>
      )}
    </div>
  );
...
export const Toast: React.FC<ToastProps> = ({ toasts = [], position = 'top-right', onDismiss, className = '' }) => (
  <div
    className={className}
    style={{ position: 'fixed', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 8, ...POSITIONS[position || 'top-right'] }}
  >
