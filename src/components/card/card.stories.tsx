    ...
    <div style={{ width: 400 }}>
      ...
      >
        <div style={{ fontFamily: 'Source Sans Pro, sans-serif', color: '#4A4A4A', fontSize: 14 }}>
          MRI Gradient Coil Assembly â Part #MRI-7842<br />
          <span style={{ color: '#777', fontSize: 13 }}>Qty: 1 Â· Unit price: $12,400.00</span>
        </div>
      </Card>
    </div>
  ),
    ...
    <div style={{ width: 340 }}>
      <Card onClick={() => alert('Opening contract details')} padding="lg">
        <div style={{ fontFamily: 'Source Sans Pro, sans-serif' }}>
          ...
          <div style={{ fontSize: 13, color: '#777' }}>Expires Dec 31, 2025 Â· Philips Brilliance CT</div>
        </div>
      </Card>
    </div>
  ),
      ...
      {(['none','sm','md','lg'] as const).map(p => (
        <Card key={p} padding={p}>
          <div style={{ fontFamily: 'Source Sans Pro, sans-serif', fontSize: 13, color: '#4A4A4A' }}>
            padding="{p}" â {p === 'none' ? '0px' : p === 'sm' ? '12px' : p === 'md' ? '16px' : '24px'}
          </div>
        </Card>
      ))}
    </div>
